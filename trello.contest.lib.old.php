...
//utils
	...
	//		Best Execution time for decodeMessage4 		\\
	//**************************************************\\
	/**
	...
	/**
	 * Function decodeMessage2
	 *
	 * Decode the message of the front-end web developper challenge : see decodeMessage
	 *  /!\ seems less performant than the brute force method
	 *
	 * @param (string) ($s) the string which has to be decoded
	 * @return (array) (theCandidate) an array which contains : the position of the 1st occurence, the position of the 2d occurence, the character, the distance between both occurences
	 */
	function decodeMessage2($s) { //do it the sequential way.
		//echo $s . $GLOBALS['nl'];
		$s=str_replace("\r\n", "", $s);
		while(list($k,$l,$c,$d)=getCandidate2($s)) {
			//echo "found : ".$c."(".$k.";".$l. ";".$d.")<br/>";	
			$s=substr_replace($s, "", $l, 1);
			$s=substr_replace($s, "", $k, 1);
			$s=$s .$c;
	 ...
	 * @return (array) (theCandidate) an array which contains : the position of the 1st occurence, the position of the 2d occurence, the character, the distance between both occurences
	 */
	function decodeMessage3($s) { //do it the sequential way.
		//echo $s . $GLOBALS['nl'];
		$s=str_replace("\r\n", "", $s);
		while(list($k,$l,$c,$d)=getCandidate3($s)) {
			//echo "found : ".$c."(".$k.";".$l. ";".$d.")<br/>";	
			$s=substr_replace($s, "", $l, 1);
			$s=substr_replace($s, "", $k, 1);
			$s=$s .$c;
	 ...
	 * @return (array) (theCandidate) an array which contains : the position of the 1st occurence, the position of the 2d occurence, the character, the distance between both occurences
	 */
	function decodeMessage4($s) { //do it the sequential way.
		//echo $s . $GLOBALS['nl'];
		$s=str_replace("\r\n", "", $s);
		while(list($k,$l,$c,$d)=getCandidate4($s)) {
			//echo "found : ".$c."(".$k.";".$l. ";".$d.")<br/>";	
			$s=substr_replace($s, "", $l, 1);
			$s=substr_replace($s, "", $k, 1);
			$s=$s .$c;
	 ...
	 * /!\ seems less performant than the brute force one
	 * Execution time: 2.4212419986725
	 * @param (string) ($s) the string which has to be searched
	 * @return (array) (theCandidate) an array which contains : the position of the 1st occurence, the position of the 2d occurence, the character, the distance between both occurences
	 */
	function getCandidate2($s) {
		$dMax=0;
		$iMax=0;
		$jMax=0;
		$cMax="";
		...
		for ($i=0;$i<strlen($s);$i++) { //from left of the string
			...
			while(($j=strpos($s, $s[$i],$j+1))!==false) { //$s[$j] ==$s[$i] && $i<>$j 
				//echo $i."-".$j ."-".$dMax."<br>";
				if (($j-$i>$dMax) && ! isTherePairs2(substr($s, $i+1,$j-$i-1))) { //no pair inside & distance is max
	...
	function getCandidate($s) {
		...
		$foundOne=false;
		for ($i=0;$i<strlen($s);$i++) { //from left of the string
			for ($j=strlen($s)-1; $j >$i ; $j--) { //from right of the string i<j
				if($s[$j] ==$s[$i]) { //found potential candidate
	...
	/**
	 * Function getCandidate3
	 *
	 * Provides the pair of identical characters that are farthest apart, and contain no pairs of identical characters between them.
	 * Mix of original and sort string >> 2.888288974762
	 *
	 * @param (string) ($s) the string which has to be searched
	 * @return (array) (theCandidate) an array which contains : the position of the 1st occurence, the position of the 2d occurence, the character, the distance between both occurences
	 */
	function getCandidate3($s) {
		$dMax=0;
		$iMax=0;
		$jMax=0;
		$cMax="";
	...
	/**
	 * Function isTherePairs2
	 *
	 * Tells if there are character duplicates in the string 
	 *
	 ...
	 */
	function isTherePairs2($s) {
	//do some sorting to accelerate the search of pairs
		$sArray=str_split($s);
		sort($sArray);
		$sSorted=implode("",$sArray);	
		for ($i=0;$i<strlen($sSorted);$i++) { //from left of the string
			if(isset($sSorted[$i+1]) && $sSorted[$i+1]==$sSorted[$i]) { //only check the next char if the string is sorted //attention au depassement
				return true;
			} 
		}
		return false;
